...
/** End of old RTC config block **/
    ...
    /* --- Audiobook mode -------------------------------------------------
     * Per-context overrides applied to the six playback/bookmark settings
     * when the currently-playing track matches the configured prefix or
     * extension list.  Master toggle defaults to off so behaviour is
     * unchanged unless the user opts in.  See apps/audiobook.h. */
    OFFON_SETTING(0, audiobook_mode, LANG_AUDIOBOOK_MODE, false,
                  "audiobook mode", NULL),
    TEXT_SETTING(0, audiobook_path, "audiobook path",
                 "/Audiobooks", NULL, NULL),
    TEXT_SETTING(0, audiobook_extensions, "audiobook extensions",
                 "m4b", NULL, NULL),
    INT_SETTING(F_TIME_SETTING, audiobook_skip_length,
                LANG_AUDIOBOOK_SKIP_LENGTH, 20,
                "audiobook skip length", UNIT_SEC, 0, 600, 5,
                NULL, NULL, NULL),
    INT_SETTING(F_TIME_SETTING, audiobook_pause_rewind,
                LANG_AUDIOBOOK_PAUSE_REWIND, 2,
                "audiobook rewind duration on pause", UNIT_SEC, 0, 60, 1,
                NULL, NULL, NULL),
    CHOICE_SETTING(0, audiobook_autocreatebookmark,
                   LANG_AUDIOBOOK_AUTOCREATE, BOOKMARK_YES,
                   "audiobook autocreate bookmarks",
                   "off,on,ask,recent only - on,recent only - ask", NULL, 5,
                   ID2P(LANG_SET_BOOL_NO), ID2P(LANG_SET_BOOL_YES),
                   ID2P(LANG_ASK), ID2P(LANG_BOOKMARK_SETTINGS_RECENT_ONLY_YES),
                   ID2P(LANG_BOOKMARK_SETTINGS_RECENT_ONLY_ASK)),
    CHOICE_SETTING(0, audiobook_autoloadbookmark,
                   LANG_AUDIOBOOK_AUTOLOAD, BOOKMARK_YES,
                   "audiobook autoload bookmarks", off_on_ask, NULL, 3,
                   ID2P(LANG_SET_BOOL_NO), ID2P(LANG_SET_BOOL_YES),
                   ID2P(LANG_ASK)),
    CHOICE_SETTING(0, audiobook_usemrb,
                   LANG_AUDIOBOOK_RECENT_BOOKMARKS,
                   BOOKMARK_ONE_PER_PLAYLIST,
                   "audiobook use most-recent-bookmarks",
                   "off,on,unique only,one per track", NULL, 4,
                   ID2P(LANG_SET_BOOL_NO), ID2P(LANG_SET_BOOL_YES),
                   ID2P(LANG_BOOKMARK_SETTINGS_ONE_PER_PLAYLIST),
                   ID2P(LANG_BOOKMARK_SETTINGS_ONE_PER_TRACK)),
#ifdef HAVE_TAGCACHE
    BOOL_SETTING(0, audiobook_autoresume_enable, LANG_AUDIOBOOK_AUTORESUME,
                 true, "audiobook autoresume enable", off_on,
                 LANG_SET_BOOL_YES, LANG_SET_BOOL_NO, NULL),
#endif
    /* --- end audiobook -------------------------------------------------- */
